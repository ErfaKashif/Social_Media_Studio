...
Platform Variant ({platform}):"""
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Local Ollama service is offline or unreachable. Please run 'ollama serve' and try again."
        )
    except httpx.HTTPStatusError as err:
        raise HTTPException(
            status_code=400,
            detail=f"Ollama generation request failed: {err.response.text}"
        )
    except Exception as err:
        raise HTTPException(
            status_code=500,
            detail=f"An unexpected error occurred during AI generation: {str(err)}"
        )
    ...
    validate_variant(platform, raw_variant_text)
